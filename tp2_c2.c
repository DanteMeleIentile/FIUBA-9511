...
double computar_alpha(double ax, double ay, double bx, double by, float px, float py){
    double norma_cuadrado = prod_interno(bx-ax, by-ay, bx-ax, by-ay);
...
double dist_punto_a_segmento(float ax, float ay, float bx, float by, float px, float py){
    ...
    double alpha = prod_interno (px-ax, py-ay, bx-ax, by-ay) / prod_interno (bx-ax, by-ay, bx-ax, by-ay); // alpha = [(P - A) . (B - A)] / [||(B - A)||^2]
...
double distancia_punto_a_polilinea(float polilinea[][2], size_t n, float px, float py){
    ...
    for (int i = 1; i < n-1; i++){
        d_temp = dist_punto_a_segmento(polilinea[i][X], polilinea[i][Y], polilinea[i+1][X], polilinea[i+1][Y], px, py);
